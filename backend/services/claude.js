...
// 프롬프트 빌더 (generateBlogPost, generateBlogPostStream 공통)
function buildBlogPrompt(transcript, options = {}) {
  const { tone = 'informative', imageCount = 3, contentLength = 'normal', customSystemPrompt, prevMarkdown } = options;
  ...
  return `다음은 블로그 포스트의 글감입니다. 음성을 텍스트로 변환한 내용이므로 다소 구어체적이거나 정리가 덜 되어 있을 수 있습니다.
...
/**
 * SSE 스트리밍 버전
 * sendChunk(text) — 청크 콜백 (text_delta 마다 호출)
 * 반환값: 전체 완성된 markdown 문자열
 */
async function generateBlogPostStream(transcript, options = {}, sendChunk) {
  const { useWebSearch = false } = options;
  const prompt = buildBlogPrompt(transcript, options);
  ...
  for await (const event of stream) {
    if (event.type === 'content_block_delta' && event.delta?.type === 'text_delta') {
      ...
      fullText += chunk;
      if (sendChunk) sendChunk(chunk);
    }
  }

  // web_search 사용 시 tool_result 이후 text 블록도 수집
  // stream.finalMessage()로 최종 확인
  ...
  // streaming 중 이미 수집했으면 그대로, 아니면 finalText 사용
...
- instagram: 정확히 5개, 영문+한글 혼합, # 기호 없이 단어만, 가장 핵심적인 키워드만
...
module.exports = { generateBlogPost, generateBlogPostStream, generateHashtags, analyzeStyle };
